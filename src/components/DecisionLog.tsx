...
export default function DecisionLog({ entries }: DecisionLogProps) {
  return (
    <div className="bg-white rounded-2xl border-2 border-[#e5e5e5] shadow-[0_4px_0_#e5e5e5] overflow-hidden">
      ...
      {entries.length === 0 ? (
        <p className="px-6 py-8 text-center text-sm font-bold text-[#afafaf]">
          ...
          {entries.map((e) => (
            <div key={e.key} className="px-6 py-3">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs font-bold text-[#777777]">{e.reason}</span>
              </div>
              {e.traceLines.length > 0 && (
                <ol className="mt-1.5 ml-4 text-[11px] font-bold text-[#666666] space-y-0.5">
